...
impl NetDevice {
    ...
    fn e1000_release_tx_ring(data: &NetDevicePrvData) {
        // 尝试获取锁
    ...
    fn e1000_release_rx_ring(data: &NetDevicePrvData) {
        // 尝试获取锁
        ...
        if let Some(mut rx_ring_buf) = rx_ring_lock.take() {
            ...
            // 比如：缓冲区需要被释放，描述符需要被清理等等
            // 因为Rust具有RAII机制，当rx_ring_buf离开作用域时，自动调用其析构函数
            // 在析构函数中可以进行相关的清理工作
            for (entry) in rx_ring.buf.borrow_mut().iter_mut() {
                if let Some((dma_map, skb)) = entry.take() {
                    drop(dma_map);
...
impl kernel::Module for E1000KernelMod {
    fn init(name: &'static CStr, module: &'static ThisModule) -> Result<Self> {
        ...
        // means the driver will be _removed.
